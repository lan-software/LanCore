...
- *Platform admin manages policies.* From `/backstage/policies` an admin creates policy types, then policies referencing those types. Each policy carries metadata (`name`, `description`, `is_required_for_registration`, `sort_order`) but no content; content lives in versions.
...
- *Admin-initiated deletion.* From `/backstage/data-lifecycle/deletion-requests` an admin holding `RequestUserDeletion` opens a request on behalf of a user (e.g. ToS violation, support ticket asking for deletion). The same email-confirmation + grace flow applies. An "Anonymize now" action skips the remaining grace once a justification is recorded.
