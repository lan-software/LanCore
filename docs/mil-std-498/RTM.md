...
| NTF-F-008 | Per-TicketType admin opt-in (`notify_on_release` / `notify_on_end` / `notify_on_end_lead_minutes`) | `Domain/Ticketing/Models/TicketType.php`, `Domain/Ticketing/Http/Requests/{Store,Update}TicketTypeRequest.php`, `resources/js/pages/ticket-types/{Create,Edit}.vue` | `Backstage/TicketTypeNotifyToggleTest.php` | Planned |
| NTF-F-009 | Per-user opt-in pair (`mail_on_ticket_sale` / `push_on_ticket_sale`) | `Domain/Notification/Models/NotificationPreference.php`, `Domain/Notification/Http/Requests/UpdateNotificationPreferencesRequest.php`, `resources/js/pages/settings/Notifications.vue` | `Settings/NotificationPreferencesTicketSaleTest.php` | Planned |
| NTF-F-010 | Scheduled `notifications:dispatch-ticket-sale` command (every 5m, idempotent via `*_notified_at` sentinel) | `Console/Commands/Notification/DispatchTicketSaleNotificationsCommand.php`, `Domain/Ticketing/Jobs/DispatchTicketSale{Release,End}Job.php`, `Domain/Ticketing/Notifications/TicketSaleNotification.php`, `routes/console.php` | `Notification/TicketSaleNotificationDispatcherTest.php` | Planned |
| NTF-F-011 | `WebPushChannel` bridges Laravel `Notification::send` → VAPID; auto-prunes 404/410 subscriptions | `Domain/Notification/Channels/WebPushChannel.php`, `Domain/Notification/Support/WebPushFactory.php`, `Providers/AppServiceProvider.php` (`Notification::extend('webpush', ...)`) | `Notification/WebPushChannelTest.php` | Planned |
| NTF-F-012 | Suppress release notifications for users already holding a ticket of that TicketType | `Domain/Ticketing/Jobs/DispatchTicketSaleReleaseJob.php` (`whereDoesntHave('tickets', ...)`) | `Notification/TicketSaleNotificationDispatcherTest.php` (suppression case) | Planned |
| NTF-F-013 | Race-protect dispatcher with `isAvailableForPurchase()` re-check inside fan-out | `Domain/Ticketing/Jobs/DispatchTicketSaleReleaseJob.php`, `DispatchTicketSaleEndJob.php`, `Domain/Ticketing/Models/TicketType.php::isAvailableForPurchase()` | `Notification/TicketSaleNotificationDispatcherTest.php` (race case) | Planned |
...
## 23. Data Lifecycle / Right to Erasure (CSCI-DL)
