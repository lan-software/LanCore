...
#### 3.2.EE Chat Domain (CSCI-CHT)

Implementation: `app/Domain/Chat/` (Actions, Broadcasting, Contracts, Enums, Events, Gdpr, Http, Listeners, Models, Notifications, Policies, Services, Support), four migrations under `database/migrations/2026_05_12_100000..100003_*`. Realtime transport: Laravel Reverb (see PLATFORM-CHT-001, `docs/deployment/reverb.md`). See DBDD §4.20.

| Req ID | Requirement |
|--------|------------|
| CHT-F-001 | The software shall provide four domain tables — `chat_rooms`, `chat_room_memberships`, `chat_messages`, `chat_moderation_actions` — with the columns and indexes specified in DBDD §4.20; all four models shall implement `OwenIt\Auditing\Contracts\Auditable` |
| CHT-F-002 | The software shall declare two domain permissions, `Permission::ManageChat` (Admin, Superadmin) and `Permission::ModerateChat` (Moderator, Admin, Superadmin), registered through `App\Enums\RolePermissionMap` |
| CHT-F-003 | The software shall expose an enum `RoomStatus` with cases `Open`, `WriteLocked`, `Archived`, controlling subscribe and post eligibility |
| CHT-F-004 | The software shall expose an enum `ModerationAction` with cases `Mute`, `Unmute`, `DeleteMessage`, `CloseRoom`, persisted on `chat_moderation_actions.action` |
| CHT-F-005 | Chat messages shall use Laravel `SoftDeletes`; the raw `body` shall be preserved on soft-delete so GDPR export (CHT-F-018) can return it, while the frontend (CHT-F-016) renders a translated placeholder |
| CHT-F-007 | The software shall expose a `RoomPolicy` contract (`canView`, `canPost`, `canModerate`, `describe`) as the sole cross-domain coupling point for the Chat CSCI; chat services shall never reach into consumer domain models directly |
| CHT-F-008 | The software shall authorize Reverb private-channel subscription on `chat.room.{roomId}` via the bound `RoomPolicy::canView`; archived rooms shall reject subscription outright; write-locked rooms shall still allow subscription so read-only viewers see the historical thread |
| CHT-F-009 | The software shall broadcast `MessagePosted` events on `chat.room.{roomId}` using `ShouldBroadcastNow` (immediate, not queued); the broadcast payload shall include the message id, room id, author id, body, mentions array, and ISO-8601 created_at |
| CHT-F-010 | The software shall expose `ChatService::ensureRoom(string $key, RoomPolicy $policy, array $attrs = []): ChatRoom` as an idempotent room-creation primitive — subsequent calls with the same key shall return the existing row without touching state |
| CHT-F-011 | The software shall expose `ChatService::writeLock(ChatRoom)` and `ChatService::archive(ChatRoom)` as state-transition primitives; `writeLock` shall be a no-op on rooms already write-locked or archived; `archive` shall be a no-op on already-archived rooms |
| CHT-F-012 | The software shall provide a `PostMessage` action that persists messages and dispatches `MessagePosted`; the action shall reject posts via translated errors on each of: WriteLocked/Archived room (`RoomNotPostableException`), policy-denied (`PostUnauthorizedException`), muted user (`UserMutedException`), exceeded short or long rate limit (`RateLimitExceededException`), duplicate within window (`DuplicateMessageException`); whitespace shall be normalized via `Str::squish` before persisting; empty bodies shall be rejected |
| CHT-F-019 | `config/chat.php` shall expose configurable rate-limit thresholds (`rate_limits.short.max` default 5 / `decay_seconds` default 10s; `rate_limits.long.max` default 30 / `decay_seconds` default 60s), a duplicate-suppression window (`duplicate_window` default 10s), and a max body length (`max_length` default 2000) |
| CHT-F-013 | The software shall parse `@username` mentions from message bodies via `MentionParser`, matching `@` followed by 3–32 alphanumeric/underscore characters, anchored to a word boundary on the left so email addresses (`foo@bar.com`) are not matched; resolution shall be case-insensitive and deduplicated |
| CHT-F-014 | `PostMessage` shall persist resolved mention user ids to `chat_messages.mentions_json` at write time |
| CHT-F-015 | The `NotifyMentionedUsers` queued listener shall dispatch `ChatMentionNotification` for each mentioned user, but shall skip (a) self-mentions and (b) users for whom the room's bound `RoomPolicy::canView` returns false |
| CHT-F-016 | `ChatMentionNotification` shall route to the `database` channel always; to `mail` when the user's `mail_on_chat_mention` preference is true (default true); to `WebPushChannel` when `push_on_chat_mention` is true (default false) |
| CHT-F-017 | The `notification_preferences` table shall be extended with boolean columns `mail_on_chat_mention` (default true) and `push_on_chat_mention` (default false) |
| CHT-F-020 | The software shall provide a `MuteUser` action which sets `chat_room_memberships.muted_until` (creating the membership if absent) and writes a `Mute` audit row; authorization shall fall back to the global `ChatPermission::ModerateChat` when the room's `RoomPolicy::canModerate` denies |
| CHT-F-021 | The software shall provide an `UnmuteUser` action which clears `muted_until` and writes an `Unmute` audit row |
| CHT-F-022 | The software shall provide a `DeleteMessage` action which soft-deletes a message, persists `deleted_by`, and writes a `DeleteMessage` audit row; the raw `body` shall remain on the row (CHT-F-005); the frontend shall render a translated placeholder for `deleted_at != null` |
| CHT-F-023 | The software shall provide a `CloseRoom` action which transitions an `Open` room to `WriteLocked` and writes a `CloseRoom` audit row; this is distinct from consumer-driven `ChatService::writeLock` (the moderator is attributed) |
| CHT-F-024 | All four moderation actions shall reject the actor via `ModerationUnauthorizedException` (403) when neither the room policy nor the global permission allows moderation |
| CHT-F-018 | The software shall expose a `ChatDataSource` GDPR data source (registered in `GdprServiceProvider::SOURCES`) which exports the subject's authored messages (including soft-deleted, with `deleted_at` and `deleted_by`) and their room memberships; other users' messages in the same rooms shall be excluded; mention user ids shall be obfuscated via `GdprExportContext` except for the subject |
| CHT-F-026 | The software shall expose a `chat:prune-archived --older-than=N [--dry-run] [--competition=ID]` artisan command which deletes chat rooms and their messages, memberships, and moderation actions for rooms whose backing competition is archived AND whose `archived_at` is older than `N` days; the command shall refuse to run without `--older-than`; `--dry-run` shall report counts without deleting; the audit-log rows on the affected models shall be preserved by laravel-auditing default behavior; the command shall not be scheduled |
| CHT-F-027 | The software shall provide a `CompetitionRoomPolicy` whose `canView` returns true for any user with confirmed team membership in the competition, `canPost` adds an `Open` room check, and `canModerate` accepts `CompetitionPermission::ManageCompetitions` or `ChatPermission::ModerateChat` |
| CHT-F-028 | The software shall observe `Competition::status` transitions and: on `RegistrationOpen` ensure the competition-wide room exists (idempotent) and auto-join all current team members; on `Finished` write-lock the room (reads preserved); on `Archived` archive the competition room AND cascade-archive every match room of the competition |
| CHT-F-029 | The software shall observe `CompetitionTeamMember::created` and auto-join the new user to the competition-wide chat room when one exists |
| CHT-F-030 | The software shall provide a `MatchRoomPolicy` whose `canView` is driven by the `chat_room_memberships` table (cheap, authoritative); `canPost` adds an `Open` room check; `canModerate` follows the same global/permission scheme as `CompetitionRoomPolicy` |
| CHT-F-031 | The software shall listen for `MatchReadyForOrchestration` and create the per-match chat room (idempotent via `ChatService::ensureRoom`); all current participants shall be auto-joined via `CompetitionRoomAutoJoin` |
| CHT-F-032 | The software shall listen for `MatchFinalized` and write-lock the corresponding match room; if no room exists the listener shall no-op without error |
