        ...
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex flex-wrap items-center gap-2">
                <div class="relative flex-1 min-w-48">
                    ...
                    <Input v-model="searchValue" placeholder="Search tickets…" class="pl-8" />
                </div>
            ...
            <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
                    ...
                    <TableBody>
                        <template v-if="table.getRowModel().rows.length">
                            <TableRow
                                ...
                                @click="router.visit(adminTicketShow(row.original.id).url)"
                            >
                ...
                <div class="flex items-center justify-between border-t border-sidebar-border/70 px-4 py-3 dark:border-sidebar-border">
                    ...
                    <div class="flex items-center gap-1">
                        <Button variant="outline" size="sm" class="size-8 p-0" :disabled="tickets.current_page <= 1" @click="setPage(tickets.current_page - 1)">
                            <ChevronLeft class="size-4" />
                        </Button>
                        <span class="px-2 text-xs text-muted-foreground">{{ tickets.current_page }} / {{ tickets.last_page }}</span>
                        <Button variant="outline" size="sm" class="size-8 p-0" :disabled="tickets.current_page >= tickets.last_page" @click="setPage(tickets.current_page + 1)">
