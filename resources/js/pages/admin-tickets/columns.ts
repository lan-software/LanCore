...
export const columns: ColumnDef<Ticket>[] = [
    {
        ...
        cell: ({ row }) => h('span', { class: 'font-medium font-mono text-sm' }, row.getValue('validation_id')),
    },
    {
        id: 'ticket_type',
        header: () => h('span', 'Type'),
        cell: ({ row }) => h('span', { class: 'text-muted-foreground' }, row.original.ticket_type?.name ?? '—'),
    },
    {
        id: 'owner',
        header: () => h('span', 'Owner'),
        cell: ({ row }) => {
            ...
            return owner
                ? h('div', { class: 'flex flex-col' }, [
                      ...
                      h('span', { class: 'text-xs text-muted-foreground' }, owner.email),
                  ])
    ...
    },
    {
        ...
        cell: ({ row }) =>
            row.original.order_id
                ? h('span', { class: 'font-mono text-sm text-muted-foreground' }, `#${row.original.order_id}`)
                : h('span', { class: 'text-muted-foreground' }, '—'),
    },
    {
        accessorKey: 'status',
