...
function isCheckboxDisabled(scrollKey: string, requiresScroll: boolean, hasContent: boolean): boolean {
...
function saveAcknowledgement(type: string, id: number, key: string | null = null) {
...
function submitCheckout() {
    ...
    router.post(CartController.checkout().url, { payment_method: props.paymentMethod }, {
        onFinish: () => {
...
<template>
    ...
    <div class="flex min-h-screen flex-col bg-background text-foreground">
        <header class="border-b">
            <div class="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
                ...
                <nav class="flex items-center gap-4">
                    <Link :href="cartShow().url" class="text-sm text-muted-foreground hover:text-foreground">
        ...
        <main class="flex-1">
            <div class="mx-auto max-w-4xl px-6 py-12">
                <div class="flex items-center gap-3 mb-8">
                    ...
                    <h1 class="text-3xl font-bold tracking-tight">Review & Confirm</h1>
                </div>
                ...
                <div class="grid gap-8 lg:grid-cols-3">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <Card v-for="req in purchaseRequirements" :key="`req-${req.id}`">
                            <CardHeader>
                                <CardTitle class="flex items-center gap-2">
                                    <FileText class="size-5" />
                                    {{ req.name }}
                                </CardTitle>
                                <p v-if="req.description" class="text-sm text-muted-foreground">{{ req.description }}</p>
                            </CardHeader>
                            <CardContent class="space-y-4">
                                <div
                                    v-if="req.requirements_content"
                                    class="max-h-48 overflow-y-auto rounded-md border bg-muted/50 p-4 text-sm prose prose-sm dark:prose-invert"
                                    v-html="req.requirements_content"
                                    @scroll="(e: globalThis.Event) => handleScroll(e, `req-${req.id}`)"
                                />

                                <div v-if="req.acknowledgements" class="space-y-3">
                                    <TooltipProvider v-for="(ack, idx) in req.acknowledgements" :key="`${req.id}-${idx}`">
                                        <div class="flex items-start gap-2">
                                            <Tooltip :disabled="!isCheckboxDisabled(`req-${req.id}`, req.requires_scroll, !!req.requirements_content)">
                                                <TooltipTrigger as-child>
                                                    <span>
                                                        <Checkbox
                                                            :id="`req-${req.id}-${idx}`"
                                                            :model-value="requirementChecks[`${req.id}-${idx}`] ?? false"
                                                            :disabled="isCheckboxDisabled(`req-${req.id}`, req.requires_scroll, !!req.requirements_content)"
                                                            @update:model-value="(val: boolean | 'indeterminate') => onRequirementCheck(req.id, idx, val)"
                                                        />
                                                    </span>
                                                </TooltipTrigger>
                                                <TooltipContent side="right">
                                                    <p>Erst lesen dann klicken</p>
                                                </TooltipContent>
                                            </Tooltip>
                                            <Label
                                                :for="`req-${req.id}-${idx}`"
                                                class="text-sm leading-relaxed"
                                                :class="isCheckboxDisabled(`req-${req.id}`, req.requires_scroll, !!req.requirements_content) ? 'text-muted-foreground cursor-not-allowed' : 'cursor-pointer'"
                                            >
                                                {{ ack }}
                                            </Label>
                                        </div>
                        ...
                        <Card v-if="globalConditions.length > 0">
                            ...
                            <CardContent class="space-y-6">
                                <div v-for="cond in globalConditions" :key="`global-${cond.id}`" class="space-y-3">
                                    <div
                                        v-if="cond.content"
                                        class="max-h-48 overflow-y-auto rounded-md border bg-muted/50 p-4 text-sm prose prose-sm dark:prose-invert"
                                        v-html="cond.content"
                                        @scroll="(e: globalThis.Event) => handleScroll(e, `global-${cond.id}`)"
                                    />
                                    <TooltipProvider>
                                        <div class="flex items-start gap-2">
                                            <Tooltip :disabled="!isCheckboxDisabled(`global-${cond.id}`, cond.requires_scroll, !!cond.content)">
                                                <TooltipTrigger as-child>
                                                    <span>
                                                        <Checkbox
                                                            :id="`global-${cond.id}`"
                                                            :model-value="globalChecks[cond.id] ?? false"
                                                            :disabled="isCheckboxDisabled(`global-${cond.id}`, cond.requires_scroll, !!cond.content)"
                                                            @update:model-value="(val: boolean | 'indeterminate') => onGlobalCheck(cond.id, val)"
                                                        />
                                                    </span>
                                                </TooltipTrigger>
                                                <TooltipContent side="right">
                                            ...
                                            <Label
                                                :for="`global-${cond.id}`"
                                                class="text-sm leading-relaxed"
                                                :class="isCheckboxDisabled(`global-${cond.id}`, cond.requires_scroll, !!cond.content) ? 'text-muted-foreground cursor-not-allowed' : 'cursor-pointer'"
                                            >
                                                {{ cond.acknowledgement_label }}
                                                <span v-if="cond.is_required" class="text-destructive">*</span>
                                            </Label>
                                        </div>
                                    </TooltipProvider>
                                </div>
                        ...
                        <Card v-if="providerConditions.length > 0">
                            <CardHeader>
                                <CardTitle>{{ paymentMethodLabel }} Conditions</CardTitle>
                            </CardHeader>
                            <CardContent class="space-y-6">
                                <div v-for="cond in providerConditions" :key="`provider-${cond.id}`" class="space-y-3">
                                    <div
                                        v-if="cond.content"
                                        class="max-h-48 overflow-y-auto rounded-md border bg-muted/50 p-4 text-sm prose prose-sm dark:prose-invert"
                                        v-html="cond.content"
                                        @scroll="(e: globalThis.Event) => handleScroll(e, `provider-${cond.id}`)"
                                    />
                                    <TooltipProvider>
                                        <div class="flex items-start gap-2">
                                            <Tooltip :disabled="!isCheckboxDisabled(`provider-${cond.id}`, cond.requires_scroll, !!cond.content)">
                                                <TooltipTrigger as-child>
                                                    <span>
                                                        <Checkbox
                                                            :id="`provider-${cond.id}`"
                                                            :model-value="providerChecks[cond.id] ?? false"
                                                            :disabled="isCheckboxDisabled(`provider-${cond.id}`, cond.requires_scroll, !!cond.content)"
                                                            @update:model-value="(val: boolean | 'indeterminate') => onProviderCheck(cond.id, val)"
                                                        />
                                                    </span>
                                                </TooltipTrigger>
                                                <TooltipContent side="right">
                                            ...
                                            <Label
                                                ...
                                                class="text-sm leading-relaxed"
                                                :class="isCheckboxDisabled(`provider-${cond.id}`, cond.requires_scroll, !!cond.content) ? 'text-muted-foreground cursor-not-allowed' : 'cursor-pointer'"
                                            >
                                                {{ cond.acknowledgement_label }}
                                                <span v-if="cond.is_required" class="text-destructive">*</span>
                                            </Label>
                        ...
                        <p v-if="!hasAnyConditions" class="text-sm text-muted-foreground">
                            No additional conditions to review. You may proceed to payment.
                        </p>
                    </div>
                        ...
                        <Card class="sticky top-6">
                            ...
                            <CardContent class="space-y-4">
                                ...
                                <div class="space-y-2 text-sm">
                                    <div v-for="item in cartItems" :key="item.name" class="flex justify-between gap-2">
                                        <span class="text-muted-foreground">
                                            {{ item.quantity }}x {{ item.name }}
                                            <Badge v-if="item.is_addon" variant="secondary" class="ml-1 text-[10px]">Addon</Badge>
                                        </span>
                                ...
                                <div class="border-t pt-3 space-y-2 text-sm">
                                    ...
                                    <div v-if="discount > 0" class="flex justify-between text-green-600">
                                        <span>Discount <span v-if="voucher" class="font-mono">({{ voucher.code }})</span></span>
                                ...
                                <Button
                                    class="w-full"
                                    size="lg"
                                    :disabled="isSubmitting || (hasAnyConditions && !allRequiredSatisfied)"
                                    @click="submitCheckout"
                                >
                                    {{ isSubmitting ? 'Processing…' : 'Confirm & Pay' }}
                                </Button>

                                <p v-if="hasAnyConditions && !allRequiredSatisfied" class="text-xs text-center text-muted-foreground">
                                    Please accept all required conditions to proceed.
                                </p>
                            </CardContent>
                        </Card>
                    </div>
        ...
        <footer class="border-t">
            <div class="mx-auto max-w-4xl px-6 py-6 text-center text-sm text-muted-foreground">
                Powered by LanCore
            </div>
        </footer>
    </div>
