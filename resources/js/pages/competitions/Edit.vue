...
const statusTransitions: Record<string, { label: string; target: string; variant: string; confirm?: string }> = {
    draft: { label: 'Open Registration', target: 'registration_open', variant: 'default' },
    registration_open: { label: 'Close Registration', target: 'registration_closed', variant: 'default', confirm: 'This will prevent new teams from joining. Continue?' },
    registration_closed: { label: 'Go Live', target: 'running', variant: 'default', confirm: 'This will start the competition. Teams will be synced to LanBrackets. Continue?' },
    running: { label: 'Mark Finished', target: 'finished', variant: 'destructive', confirm: 'This will end the competition. Are you sure?' },
...
function submitTransition() {
    ...
    transitionForm.patch(CompetitionController.update(props.competition.id).url, {
...
const teamProgress = computed(() =>
    maxTeams.value > 0 ? Math.min(100, (teamCount.value / maxTeams.value) * 100) : 0,
);
...
function deleteCompetition() {
    if (!window.confirm(`Delete "${props.competition.name}"? This cannot be undone.`)) {
        return;
    }
    deleting.value = true;
                ...
                <div class="flex items-center justify-between">
                    <template v-for="(s, i) in allStatuses" :key="s.key">
                        <div class="flex flex-col items-center gap-1.5">
                            ...
                            >
                                <CheckCircle2 v-if="i < currentStatusIndex" class="size-4" />
                                <Play v-else-if="i === currentStatusIndex && s.key === 'running'" class="size-3.5" />
                                <Trophy v-else-if="i === currentStatusIndex && s.key === 'finished'" class="size-3.5" />
                                <span v-else>{{ i + 1 }}</span>
                            </div>
                            <span
                                class="text-[10px] font-medium"
                                :class="i <= currentStatusIndex ? 'text-foreground' : 'text-muted-foreground'"
                            >
                                {{ s.label }}
                            </span>
                        </div>
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    >
                        <Heading
                            ...
                            :description="canEditDetails ? 'Edit competition details' : 'Competition details (read-only after registration closes)'"
                        />

                        <div class="grid gap-2">
                            <Label for="name">Name</Label>
                            <Input id="name" name="name" :default-value="competition.name" :disabled="!canEditDetails" />
                            <InputError :message="errors.name" />
                        </div>

                        <div class="grid gap-2">
                            <Label for="description">Description</Label>
                            <Textarea id="description" name="description" :default-value="competition.description ?? ''" rows="3" :disabled="!canEditDetails" />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="grid gap-2">
                                ...
                                >
                                    ...
                                    <option v-for="game in games" :key="game.id" :value="game.id" :selected="game.id === competition.game_id">
                                        {{ game.name }}
                                    </option>
                                </select>
                            ...
                            <div class="grid gap-2">
                                ...
                                >
                                    <option value="">None</option>
                                    <option v-for="event in events" :key="event.id" :value="event.id" :selected="event.id === competition.event_id">
                                        {{ event.name }}
                                    </option>
                                </select>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="grid gap-2">
                                ...
                                <Input id="team_size" name="team_size" type="number" min="1" :default-value="String(competition.team_size ?? '')" :disabled="!canEditDetails" />
                                <InputError :message="errors.team_size" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="max_teams">Max Teams</Label>
                                <Input id="max_teams" name="max_teams" type="number" min="2" :default-value="String(competition.max_teams ?? '')" :disabled="!canEditDetails" />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="grid gap-2">
                                ...
                                <Input id="starts_at" name="starts_at" type="datetime-local" :default-value="competition.starts_at?.slice(0, 16) ?? ''" :disabled="!canEditDetails" />
                                <InputError :message="errors.starts_at" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="ends_at">Ends At</Label>
                                <Input id="ends_at" name="ends_at" type="datetime-local" :default-value="competition.ends_at?.slice(0, 16) ?? ''" :disabled="!canEditDetails" />
                                <InputError :message="errors.ends_at" />
                            </div>
                        </div>

                        <div v-if="canEditDetails" class="flex items-center gap-4">
                ...
                <div class="space-y-4">
                    ...
                    <div class="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                        ...
                        <Badge :class="statusColor(competition.status)" class="mb-3 capitalize">
                        ...
                        <div v-if="transition" class="mt-2">
                            ...
                            >
                                <Loader2 v-if="transitioning" class="mr-1.5 size-4 animate-spin" />
                                {{ transition.label }}
                            </Button>
                            <p v-if="transition.confirm" class="mt-2 text-[11px] text-muted-foreground">
                                <AlertTriangle class="mr-0.5 inline size-3" />
                                {{ competition.status === 'running' ? 'This will end the competition.' : 'This action cannot be undone.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                        <div class="mb-3 flex items-center justify-between">
                            ...
                            <span class="text-xs text-muted-foreground">
                                {{ teamCount }}<template v-if="maxTeams"> / {{ maxTeams }}</template>
                            </span>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    <span class="font-medium">
                                        {{ team.name }}
                                        <span v-if="team.tag" class="text-muted-foreground">[{{ team.tag }}]</span>
                                    </span>
                            ...
                            <p v-if="!competition.teams?.length" class="text-xs text-muted-foreground">
                    ...
                    <!-- LanBrackets -->
                    <div v-if="lanbracketsEnabled" class="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                        <h3 class="mb-3 text-sm font-semibold">LanBrackets</h3>
                        <div class="space-y-2">
                            <template v-if="competition.lanbrackets_id">
                                <Badge variant="outline" class="border-green-300 text-green-700 dark:border-green-700 dark:text-green-400">
                                ...
                                <div class="mt-2 space-y-1.5">
                                    ...
                                    <a
                                        ...
                                        class="flex items-center gap-1 text-sm text-primary hover:underline"
                                    >
                                        <ExternalLink class="size-3" /> Bracket View
                                    </a>
                                </div>
                            </template>
                            <p v-else class="text-xs text-muted-foreground">
                                Not synced yet. Sync happens automatically after creation.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                        ...
                        <dl class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <dd class="capitalize">{{ competition.type }}</dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-muted-foreground">Format</dt>
                                <dd class="capitalize">{{ competition.stage_type?.replace(/_/g, ' ') }}</dd>
                            </div>
                            <div v-if="competition.game" class="flex justify-between">
                                <dt class="text-muted-foreground">Game</dt>
                                <dd>{{ competition.game.name }}</dd>
                            </div>
                            <div v-if="competition.starts_at" class="flex justify-between">
                                <dt class="text-muted-foreground">Starts</dt>
                                <dd class="text-xs">{{ new Date(competition.starts_at).toLocaleString() }}</dd>
                            </div>
                            <div v-if="competition.ends_at" class="flex justify-between">
                                <dt class="text-muted-foreground">Ends</dt>
                                <dd class="text-xs">{{ new Date(competition.ends_at).toLocaleString() }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
