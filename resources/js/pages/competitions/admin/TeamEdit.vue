...
import { ArrowLeft, Check, Crown, Mail, Trash2, Users, X } from 'lucide-vue-next';
import { ref } from 'vue';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.team.name, href: AdminTeamController.edit({ team: props.team.id }).url },
];

function removeMember(member: Member) {
    if (!window.confirm(`Remove ${member.name} from the team?`)) return;
    router.delete(`/admin/teams/${props.team.id}/members/${member.id}`, { preserveScroll: true });
}
...
function deleteTeam() {
    if (!window.confirm(`Delete team "${props.team.name}"? This cannot be undone.`)) return;
    if (!props.team.competition) return;
    router.delete(`/competitions/${props.team.competition.id}/teams/${props.team.id}`);
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            <div class="flex items-center justify-between">
                <Link :href="adminTeamsRoute().url" class="text-sm text-muted-foreground hover:text-foreground">
                    <ArrowLeft class="mr-1 inline size-3" /> Back to Teams
                </Link>
                <Button variant="ghost" size="sm" class="text-destructive hover:text-destructive" @click="deleteTeam">
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                <div class="space-y-6 lg:col-span-2">
                    <!-- Team details form -->
                    <Form
                        v-bind="AdminTeamController.update.form({ team: team.id })"
                        class="space-y-6"
                        v-slot="{ errors, processing }"
                    >
                        <Heading variant="small" :title="team.name" description="Edit team details" />

                        <div class="grid grid-cols-2 gap-4">
                            <div class="grid gap-2">
                                <Label for="name">Name</Label>
                                <Input id="name" name="name" :default-value="team.name" />
                                <InputError :message="errors.name" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="tag">Tag</Label>
                                <Input id="tag" name="tag" :default-value="team.tag ?? ''" maxlength="10" />
                        ...
                        <div class="space-y-3">
                            ...
                            >
                                ...
                                <div class="flex items-center gap-2">
                                    <span v-if="member.joined_at" class="text-xs text-muted-foreground">
                            ...
                            <p v-if="team.members.length === 0" class="text-xs text-muted-foreground">No members.</p>
                        </div>
                    </div>

                    <!-- Pending Join Requests -->
                    <div v-if="team.pending_requests.length > 0" class="rounded-xl border border-blue-200 bg-blue-50/50 p-5 dark:border-blue-900 dark:bg-blue-950/30">
                        <h2 class="mb-4 text-sm font-semibold">Join Requests ({{ team.pending_requests.length }})</h2>
                        <div class="space-y-3">
                            <div
                            ...
                            >
                                ...
                                <div class="flex items-center gap-2">
                                    <Button size="sm" variant="outline" class="text-green-600" @click="resolveRequest(req.id, 'approve')">
                                        <Check class="mr-1 size-3" /> Approve
                                    </Button>
                                    <Button size="sm" variant="outline" class="text-red-600" @click="resolveRequest(req.id, 'deny')">
                ...
                <div class="space-y-4">
                    <div class="rounded-xl border p-4">
                        ...
                        <div v-if="team.competition" class="space-y-2 text-sm">
                            <div class="font-medium">{{ team.competition.name }}</div>
                            <Badge :class="statusColor(team.competition.status)" class="text-[10px] capitalize">
                                {{ team.competition.status.replace(/_/g, ' ') }}
                            </Badge>
                            <div v-if="team.competition.game" class="text-xs text-muted-foreground">{{ team.competition.game }}</div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border p-4">
                        ...
                        <dl class="space-y-2 text-sm">
                            ...
                            <div class="flex justify-between">
                                <dt class="text-muted-foreground">Pending Requests</dt>
                                <dd>{{ team.pending_requests.length }}</dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-muted-foreground">Pending Invites</dt>
                                <dd>{{ team.pending_invites.length }}</dd>
                            </div>
                        </dl>
                    </div>

                    <div v-if="team.pending_invites.length > 0" class="rounded-xl border p-4">
                        <h3 class="mb-3 text-sm font-semibold">Pending Invites</h3>
                        <div class="space-y-2">
                            <div
