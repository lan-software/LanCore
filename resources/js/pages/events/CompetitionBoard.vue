...
const conflictedScheduleIds = computed(() => {
    ...
    for (const c of conflicts.value) {
        for (const id of c.schedule_ids) {
            set.add(id);
        }
    }
...
useEcho(
    `event.${props.event.id}.competition-board`,
...
function sync() {
    router.post(
        `/backstage/competition-board/sync`,
        {},
        { preserveScroll: true },
    );
}
function autoFit() {
    router.post(
        `/backstage/competition-board/auto-fit`,
        {},
        { preserveScroll: true },
    );
}
...
<template>
    <AppLayout>
        ...
        <div class="space-y-3 p-4">
            <Heading
                :title="t('competition_board.title')"
                :description="t('competition_board.subtitle')"
            />
            ...
            <div v-if="conflicts.length > 0" class="space-y-1">
                <Alert
                    ...
                    :key="idx"
                    :variant="
                        c.severity === 'error' ? 'destructive' : 'default'
                    "
                >
                    <AlertDescription>{{
                        t(c.message_key, c.params)
                    }}</AlertDescription>
                </Alert>
            </div>

            <div
                class="rounded-md border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950"
            >
                <div class="overflow-x-auto">
                    ...
                    >
                        <div
                            class="sticky left-0 z-20 flex w-48 items-center bg-zinc-50 px-3 text-[11px] font-semibold tracking-wide text-zinc-500 uppercase dark:bg-zinc-900"
                        >
                            {{ t('competition_board.column.competition') }}
                        </div>
                        <div
                            class="sticky left-48 z-20 flex w-20 items-center justify-center border-l border-zinc-200 bg-zinc-50 px-2 text-[11px] font-semibold tracking-wide text-zinc-500 uppercase dark:border-zinc-800 dark:bg-zinc-900"
                        >
                    ...
                    <CompetitionRow
                        ...
                        @stage-drag-end="
                            (p) =>
                                patchStage(
                                    p.stageId,
                                    { starts_at: p.newStartIso },
                                    (s) => (s.starts_at = p.newStartIso),
                                )
                        "
                        @stage-resize-duration-end="
                            (p) =>
                                patchStage(
                                    p.stageId,
                                    { estimated_duration_minutes: p.minutes },
                                    (s) =>
                                        (s.estimated_duration_minutes =
                                            p.minutes),
                                )
                        "
                        @stage-resize-reserve-end="
                            (p) =>
                                patchStage(
                                    p.stageId,
                                    { reserve_buffer_minutes: p.minutes },
                                    (s) =>
                                        (s.reserve_buffer_minutes = p.minutes),
                                )
                        "
                        @round-drag-end="
                            (p) =>
                                patchRound(
                                    p.roundId,
                                    { starts_at: p.newStartIso },
                                    (r) => (r.starts_at = p.newStartIso),
                                )
                        "
                        @round-resize-duration-end="
                            (p) =>
                                patchRound(
                                    p.roundId,
                                    { estimated_duration_minutes: p.minutes },
                        ...
                        "
                        @round-resize-reserve-end="
                            (p) =>
                                patchRound(
                                    p.roundId,
                                    { reserve_buffer_minutes: p.minutes },
                                    (r) =>
                                        (r.reserve_buffer_minutes = p.minutes),
                                )
                        "
                    />
                </div>
