...
const { segments, totalWidth, rangeStart, ticks, xForTime, minuteForX } = useTimeAxis(
    () => props.event,
    () => competitions.value,
);
...
function refreshData() {
    fetch(`/backstage/competition-board/data`, {
            ...
            <div class="rounded-md border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
                <div class="overflow-x-auto">
                    ...
                    <CompetitionRow
                        ...
                        @drag-end="(p) => patchStage(p.stageId, { starts_at: p.newStartIso }, (s) => (s.starts_at = p.newStartIso))"
                        @resize-duration-end="(p) => patchStage(p.stageId, { estimated_duration_minutes: p.minutes }, (s) => (s.estimated_duration_minutes = p.minutes))"
                        @resize-reserve-end="(p) => patchStage(p.stageId, { reserve_buffer_minutes: p.minutes }, (s) => (s.reserve_buffer_minutes = p.minutes))"
                    />
                </div>
            </div>
        </div>
