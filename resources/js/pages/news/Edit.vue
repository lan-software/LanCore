...
import {
    update,
    destroy,
} from '@/actions/App/Domain/News/Http/Controllers/NewsArticleController';
import {
...
const publishMode = ref<'none' | 'now' | 'schedule'>(
    props.article.published_at ? 'schedule' : 'none',
);
...
function deleteComment(comment: NewsComment) {
    router.delete(destroyComment({ newsComment: comment.id }).url, {
        preserveScroll: true,
    });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full max-w-3xl flex-1 flex-col gap-8 p-4">
            <div class="flex items-center justify-between">
                ...
                <a
                    v-if="
                        article.visibility === 'public' && article.published_at
                    "
            ...
            <form @submit.prevent="submit" class="space-y-8">
                ...
                <div class="space-y-4">
                    <Heading
                        ...
                        description="Edit the article details"
                    />
                    ...
                    <div class="grid gap-2">
                        ...
                        <Textarea
                            ...
                            rows="3"
                            placeholder="Brief summary shown on the welcome page…"
                        />
                        <p class="text-xs text-muted-foreground">
                            Max 500 characters. Displayed as a preview on the
                            homepage.
                        </p>
                ...
                <div class="space-y-4">
                    <Heading
                        variant="small"
                        title="Publishing"
                        description="Control visibility and publishing settings"
                    />
                    ...
                    <div class="grid gap-2">
                        ...
                        <Select v-model="publishMode">
                            <SelectTrigger>
                                <SelectValue
                                    placeholder="Select publishing behavior"
                                />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem value="none"
                                    >No publish date</SelectItem
                                >
                                <SelectItem value="now">Publish Now</SelectItem>
                                <SelectItem value="schedule"
                                    >Schedule for later</SelectItem
                                >
                            </SelectContent>
                        </Select>
                        <p
                            v-if="publishMode === 'now'"
                            class="text-xs text-muted-foreground"
                        >
                            The article will be published immediately with the
                            current date and time.
                        </p>
                        <p
                            v-if="publishMode === 'none'"
                            class="text-xs text-muted-foreground"
                        >
                            The article will not have a publish date. Set one
                            later to publish.
                        </p>
                    </div>
                    ...
                    <div v-if="publishMode === 'schedule'" class="grid gap-2">
                        ...
                        />
                        <p class="text-xs text-muted-foreground">
                            The article will become visible at this date and
                            time.
                        </p>
                ...
                <div class="space-y-4">
                    <Heading
                        variant="small"
                        title="Comments"
                        description="Configure comment settings for this article"
                    />

                    <div class="flex items-center gap-2">
                        <Checkbox
                            id="comments_enabled"
                            v-model:checked="form.comments_enabled"
                        />
                        <Label
                            for="comments_enabled"
                    ...
                    <div class="flex items-center gap-2">
                        <Checkbox
                            ...
                            v-model:checked="form.comments_require_approval"
                        />
                        <Label
                            for="comments_require_approval"
                            class="text-sm font-normal"
                            >Comments require admin approval</Label
                        >
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                        <Checkbox
                            id="notify_users"
                            v-model:checked="form.notify_users"
                        />
                        <Label for="notify_users" class="text-sm font-normal"
                            >Notify users when published</Label
                        >
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <Heading
                        variant="small"
                        title="SEO & Social"
                        description="Optimize search engine and social media presence"
                    />
            ...
            >
                <div
                    class="max-w-sm space-y-4 rounded-lg bg-background p-6 shadow-lg"
                >
            ...
            >
                <Heading
                    variant="small"
                    title="Comments"
                    :description="`${article.comments.length} comment(s)`"
                />
