...
export const columns: ColumnDef<NewsComment & { article?: { id: number; title: string; slug: string; visibility: string; tags: string[] | null } }>[] = [
    {
        id: 'content',
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'max-w-md' }, [
                ...
                row.original.edited_at
                    ? h('span', { class: 'text-xs text-muted-foreground italic' }, '(edited)')
                    : null,
            ])
        },
    },
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'space-y-0.5' }, [
                ...
                h(
                    Badge,
                    { variant: article.visibility === 'public' ? 'default' : article.visibility === 'internal' ? 'secondary' : 'outline', class: 'text-[10px]' },
                    () => article.visibility.charAt(0).toUpperCase() + article.visibility.slice(1),
                ),
            ])
        },
    },
