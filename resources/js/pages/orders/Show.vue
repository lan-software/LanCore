...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Order #${props.order.id}`, href: OrderController.show(props.order.id).url },
]
...
function paymentMethodLabel(method: string): string {
    return method === 'stripe' ? 'Credit Card (Stripe)' : method === 'on_site' ? 'Pay on Site' : method
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4 max-w-4xl">
            <div>
                <Link :href="ordersIndex().url" class="text-sm text-muted-foreground hover:text-foreground">
                    &larr; Back to Orders
                </Link>
            </div>
                ...
                <CardContent>
                    <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm">{{ paymentMethodLabel(order.payment_method) }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-muted-foreground">Subtotal</dt>
                            <dd class="mt-1 text-sm">{{ formatCurrency(order.subtotal) }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-muted-foreground">Discount</dt>
                            <dd class="mt-1 text-sm">{{ order.discount > 0 ? '−' + formatCurrency(order.discount) : '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-muted-foreground">Total</dt>
                            <dd class="mt-1 text-sm font-semibold">{{ formatCurrency(order.total) }}</dd>
                        </div>
                        <div v-if="order.provider_transaction_id">
                            <dt class="text-sm font-medium text-muted-foreground">Transaction ID</dt>
                            <dd class="mt-1 text-sm font-mono">{{ order.provider_transaction_id }}</dd>
                        </div>
                    </dl>
                </CardContent>
            </Card>
                ...
                <CardContent>
                    <div class="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" as-child>
                            <Link :href="userShow(order.user.id).url">View User</Link>
                        </Button>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card v-if="order.event">
                ...
                <CardContent>
                    <div class="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" as-child>
                            <Link :href="eventEdit(order.event.id).url">View Event</Link>
                        </Button>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card v-if="order.voucher">
                ...
                <CardContent>
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="font-medium font-mono">{{ order.voucher.code }}</p>
                            <p class="text-sm text-muted-foreground">
                                <template v-if="order.voucher.type === 'fixed_amount'">
                                    {{ formatCurrency(order.voucher.discount_amount!) }} off
                                </template>
                                <template v-else>
            ...
            <Card v-if="order.order_lines && order.order_lines.length > 0">
                ...
                <CardContent class="p-0">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-for="line in order.order_lines" :key="line.id">
                                ...
                                <TableCell class="text-right">{{ formatCurrency(line.total_price) }}</TableCell>
                            </TableRow>
                        </TableBody>
                    </Table>
                </CardContent>
            ...
            <Card v-if="order.tickets && order.tickets.length > 0">
                ...
                <CardContent class="p-0">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-for="ticket in order.tickets" :key="ticket.id">
                                <TableCell class="font-mono text-sm">{{ ticket.validation_id }}</TableCell>
                                <TableCell>
                                    <Link
                                        v-if="ticket.ticket_type"
                                        :href="ticketTypeEdit(ticket.ticket_type_id).url"
