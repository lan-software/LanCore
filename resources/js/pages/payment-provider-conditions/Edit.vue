...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: PaymentProviderConditionController.edit(props.condition.id).url },
]

function deleteCondition() {
    if (confirm('Are you sure you want to delete this condition?')) {
        router.delete(PaymentProviderConditionController.destroy(props.condition.id).url)
    }
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-8 p-4 max-w-2xl">
            ...
            <Form v-bind="PaymentProviderConditionController.update.form(condition.id)" class="space-y-8" v-slot="{ errors, processing }">
                <div class="space-y-4">
                    <Heading variant="small" title="Condition Details" description="Edit the payment provider condition" />

                    <div class="grid gap-2">
                        <Label for="payment_method">Payment Method</Label>
                        <Select name="payment_method" :default-value="condition.payment_method">
                            <SelectTrigger>
