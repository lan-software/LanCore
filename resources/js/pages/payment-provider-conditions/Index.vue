...
const columns: ColumnDef<ConditionRow>[] = [
    { accessorKey: 'name', header: 'Name' },
    {
        ...
        cell: ({ row }) =>
            h(Badge, { variant: 'outline' }, () => row.original.payment_method),
    },
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex flex-wrap items-center justify-between gap-2">
                ...
                <Button as-child>
                    <Link
                        :href="PaymentProviderConditionController.create().url"
                    >
                        <Plus class="size-4" />
                        Create Condition
                    </Link>
            ...
            >
                    ...
                    <TableBody>
                        <template v-if="table.getRowModel().rows.length">
                            <TableRow
                                ...
                                @click="
                                    router.visit(
                                        PaymentProviderConditionController.edit(
                                            row.original.id,
                                        ).url,
                                    )
                                "
                ...
                >
                    <span class="text-xs text-muted-foreground"
                        >{{ conditions.total }} conditions</span
                    >
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="sm"
                            class="size-8 p-0"
                            :disabled="conditions.current_page <= 1"
                            @click="setPage(conditions.current_page - 1)"
                        >
                            <ChevronLeft class="size-4" />
                        </Button>
                        <span class="px-2 text-xs text-muted-foreground"
                            >{{ conditions.current_page }} /
                            {{ conditions.last_page }}</span
                        >
                        <Button
                            variant="outline"
                            size="sm"
                            class="size-8 p-0"
                            :disabled="
                                conditions.current_page >= conditions.last_page
                            "
                            @click="setPage(conditions.current_page + 1)"
                        >
