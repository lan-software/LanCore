...
<template>
    <AppLayout>
        ...
        <div class="space-y-4 p-4">
            <Heading
                :title="t('play_next.title')"
                :description="event?.name ?? t('play_next.no_event')"
            />
            ...
            <Card v-if="top">
                <CardContent class="space-y-3 p-4">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <Badge
                            :variant="top.blocked ? 'destructive' : 'secondary'"
                        >
                            {{
                                top.blocked
                                    ? t('play_next.blocked')
                    ...
                    <div class="text-sm">
                        <template
                            v-for="(part, idx) in top.participants"
                            :key="idx"
                        >
                            <span>{{ part.name || '?' }}</span>
                            <span v-if="idx < top.participants.length - 1">
                                vs
                            </span>
                        </template>
                    </div>
