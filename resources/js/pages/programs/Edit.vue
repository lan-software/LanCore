...
import {
    ...
    update,
    destroy,
} from '@/actions/App/Domain/Program/Http/Controllers/ProgramController';
...
function onPrimaryToggle(checked: boolean | 'indeterminate') {
    ...
    if (
        checked &&
        currentEvent.value?.primary_program &&
        currentEvent.value.primary_program.id !== props.program.id
    ) {
...
function toggleProgramSponsor(
    ...
    if (checked === true) {
        ...
        programSponsorIds.value = programSponsorIds.value.filter(
            (id) => id !== sponsorId,
        );
    }
}
...
function toggleSlotSponsor(
    ...
    if (checked === true) {
        ...
        timeSlots.value[slotIndex].sponsor_ids = ids.filter(
            (id) => id !== sponsorId,
        );
    }
}
        ...
        <div class="flex h-full max-w-2xl flex-1 flex-col gap-8 p-4">
            ...
            >
                ...
                <div class="space-y-4">
                    ...
                    />
                    <p
                        v-if="programSponsorIds.length > 0"
                        class="text-sm text-muted-foreground italic"
                    >
                        presented by
                        {{
                            sponsors
                                .filter((s) => programSponsorIds.includes(s.id))
                                .map((s) => s.name)
                                .join(', ')
                        }}
                    </p>
                    ...
                    <div class="grid gap-2">
                        ...
                        <Select
                            name="visibility"
                            :default-value="program.visibility"
                        >
                            <SelectTrigger>
                    ...
                    <div class="grid gap-2">
                        ...
                        >
                            <Checkbox
                                ...
                                :model-value="
                                    programSponsorIds.includes(sponsor.id)
                                "
                                @update:model-value="
                                    (checked: boolean | 'indeterminate') =>
                                        toggleProgramSponsor(
                                            sponsor.id,
                                            checked,
                                        )
                                "
                            ...
                            />
                            <Label
                                :for="`program_sponsor_${sponsor.id}`"
                            ...
                            >
                                ...
                                <span
                                    ...
                                    >({{ sponsor.sponsor_level.name }})</span
                                >
                            </Label>
                        </div>
                    </div>
                    ...
                    >
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                >
                                <span
                                    v-if="slot.sponsor_ids.length > 0"
                                ...
                                >
                                    ...
                                    {{
                                        sponsors
                                            .filter((s) =>
                                                slot.sponsor_ids.includes(s.id),
                                            )
                                            .map((s) => s.name)
                                            .join(', ')
                                    }}
                        ...
                        <div v-if="sponsors.length > 0" class="space-y-2">
                            ...
                            <div class="flex flex-wrap gap-x-4 gap-y-2">
                                ...
                                >
                                    ...
                                    />
                                    <Label
                                        :for="`time_slots_${index}_sponsor_${sponsor.id}`"
                            ...
                            <InputError
                                :message="
                                    errors[`time_slots.${index}.sponsor_ids`]
                                "
                            />
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">
                            Permanently delete this program and all its time
                            slots.
                        </p>
                    </div>
                    <Button
        ...
        <Dialog v-model:open="showPrimaryConfirmDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        The event "{{ currentEvent?.name }}" already has "{{
                            currentEvent?.primary_program?.name
        ...
        <Dialog v-model:open="showDeleteDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        This action cannot be undone. The program and all its
                        time slots will be permanently removed.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
