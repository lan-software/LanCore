...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: PurchaseRequirementController.edit(props.requirement.id).url },
]

const acknowledgements = ref<string[]>(props.requirement.acknowledgements?.length ? [...props.requirement.acknowledgements] : [''])

const selectedTicketTypeIds = ref<number[]>(props.requirement.ticket_types.map((t) => t.id))
const selectedAddonIds = ref<number[]>(props.requirement.addons.map((a) => a.id))
...
function deleteRequirement() {
    if (confirm('Are you sure you want to delete this purchase requirement?')) {
        router.delete(PurchaseRequirementController.destroy(props.requirement.id).url)
    }
}
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-8 p-4 max-w-2xl">
            <div class="flex items-center justify-between">
                ...
                <Button variant="destructive" size="sm" @click="deleteRequirement">Delete</Button>
            </div>

            <Form v-bind="PurchaseRequirementController.update.form(requirement.id)" class="space-y-8" v-slot="{ errors, processing }">
                <div class="space-y-4">
                    <Heading variant="small" title="Requirement Details" description="Edit the purchase requirement" />

                    <div class="grid gap-2">
                        <Label for="name">Name</Label>
                        <Input id="name" name="name" required :default-value="requirement.name" />
                        <InputError :message="errors.name" />
                    </div>

                    <div class="grid gap-2">
                        <Label for="description">Description</Label>
                        <Input id="description" name="description" :default-value="requirement.description ?? ''" />
                    ...
                    <div class="flex items-center gap-2">
                        <Checkbox id="is_active" name="is_active" :default-value="requirement.is_active" />
                        <Label for="is_active" class="cursor-pointer">Active</Label>
                    </div>
                    <InputError :message="errors.is_active" />

                    <div class="flex items-center gap-2">
                        <Checkbox id="requires_scroll" name="requires_scroll" :default-value="requirement.requires_scroll" />
                ...
                <div class="space-y-4">
                    <Heading variant="small" title="Acknowledgements" description="Checkboxes the buyer must accept" />
                        ...
                        <div class="grid gap-1 max-h-48 overflow-y-auto rounded-md border p-3">
                            <div v-for="tt in ticketTypes" :key="tt.id" class="flex items-center gap-2">
                                <Checkbox
                                    ...
                                    :value="String(tt.id)"
                                    :default-value="selectedTicketTypeIds.includes(tt.id)"
                        ...
                        <div class="grid gap-1 max-h-48 overflow-y-auto rounded-md border p-3">
                            <div v-for="addon in addons" :key="addon.id" class="flex items-center gap-2">
                                <Checkbox
                                    ...
                                    :default-value="selectedAddonIds.includes(addon.id)"
