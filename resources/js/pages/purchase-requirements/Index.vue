...
const columns: ColumnDef<PurchaseRequirementRow>[] = [
    ...
    },
    {
        ...
        header: 'Associations',
        cell: ({ row }) =>
            `${row.original.ticket_types_count} tickets, ${row.original.addons_count} addons`,
    },
];
                    ...
                    <TableBody>
                        <template v-if="table.getRowModel().rows.length">
                            <TableRow
                                ...
                                @click="
                                    router.visit(
                                        PurchaseRequirementController.edit(
                                            row.original.id,
                                        ).url,
                                    )
                                "
                ...
                >
                    <span class="text-xs text-muted-foreground">
                        <template v-if="requirements.from && requirements.to">
                            Showing {{ requirements.from }}–{{
                                requirements.to
                            }}
                            of {{ requirements.total }}
                        </template>
                        <template v-else
                            >{{ requirements.total }} requirements</template
                        >
                    </span>
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="sm"
                            class="size-8 p-0"
                            :disabled="requirements.current_page <= 1"
                            @click="setPage(requirements.current_page - 1)"
                        >
                            <ChevronLeft class="size-4" />
                        </Button>
                        <span class="px-2 text-xs text-muted-foreground"
                            >{{ requirements.current_page }} /
                            {{ requirements.last_page }}</span
                        >
                        <Button
                            variant="outline"
                            size="sm"
