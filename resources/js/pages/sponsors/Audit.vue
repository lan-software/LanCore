...
import { edit as sponsorEdit } from '@/actions/App/Domain/Sponsoring/Http/Controllers/SponsorController';
...
<template>
    <Head :title="`Audit Log – ${sponsor.name}`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold">
                    Audit Log for {{ sponsor.name }}
                </h2>
                <Button variant="outline" as-child>
                    <Link :href="sponsorEdit(sponsor.id).url"
                        >Back to Sponsor</Link
                    >
                </Button>
            </div>
