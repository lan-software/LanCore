...
import { edit as sponsorEdit, update, destroy } from '@/actions/App/Domain/Sponsoring/Http/Controllers/SponsorController'
        ...
        <div class="flex h-full flex-1 flex-col gap-8 p-4 max-w-2xl">
            ...
            >
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid gap-2">
                        ...
                        <div v-if="sponsor.logo_url && !removeLogo" class="flex items-center gap-4">
                            <img
                    ...
                    <div class="grid gap-2">
                        ...
                        <Select name="sponsor_level_id" :default-value="sponsor.sponsor_level_id ? String(sponsor.sponsor_level_id) : undefined">
                            <SelectTrigger>
                ...
                <div class="space-y-4">
                    ...
                    <div v-if="users.length > 0" class="space-y-2">
                        <div v-for="user in users" :key="user.id" class="flex items-center gap-2">
                            ...
                            />
                            <Label :for="`manager-${user.id}`" class="cursor-pointer">
                                {{ user.name }} <span class="text-muted-foreground">({{ user.email }})</span>
                            </Label>
                        </div>
                    </div>
                    <p v-else class="text-sm text-muted-foreground">No users available.</p>
                </div>
            ...
            <div class="border-t pt-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-medium text-destructive">Delete Sponsor</h3>
                        <p class="text-sm text-muted-foreground">Permanently delete this sponsor and remove all event associations.</p>
                    </div>
                    <Button
                        variant="destructive"
                        size="sm"
        ...
        <Dialog v-model:open="showDeleteDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        This action cannot be undone. The sponsor will be permanently removed from all events.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button
