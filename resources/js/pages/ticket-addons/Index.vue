                    ...
                    <TableBody>
                        <template v-if="table.getRowModel().rows.length">
                            <TableRow
                                ...
                                @click="
                                    router.visit(
                                        AddonController.edit(row.original.id)
                                            .url,
                                    )
                                "
                            >
                ...
                >
                    <span class="text-xs text-muted-foreground">
                        <template v-if="ticketAddons.from && ticketAddons.to">
                            Showing {{ ticketAddons.from }}–{{
                                ticketAddons.to
                            }}
                            of {{ ticketAddons.total }} addons
                        </template>
                        <template v-else
                            >{{ ticketAddons.total }} addons</template
                        >
                    </span>
                    <div class="flex items-center gap-1">
                        <Button
                            ...
                            size="sm"
                            class="size-8 p-0"
                            :disabled="ticketAddons.current_page <= 1"
                            @click="setPage(ticketAddons.current_page - 1)"
                        >
                            <ChevronLeft class="size-4" />
                        </Button>
                        <span class="px-2 text-xs text-muted-foreground"
                            >{{ ticketAddons.current_page }} /
                            {{ ticketAddons.last_page }}</span
                        >
                        <Button
                            variant="outline"
                            size="sm"
