        ...
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <Select
                    ...
                    @update:model-value="(val) => setFilter('event_id', val === 'all' ? undefined : String(val))"
                >
                    ...
                    <TableBody>
                        <template v-if="table.getRowModel().rows.length">
                            <TableRow
                                ...
                                @click="router.visit(TicketTypeController.edit(row.original.id).url)"
                            >
                                <TableCell
                ...
                <div class="flex items-center justify-between border-t border-sidebar-border/70 px-4 py-3 dark:border-sidebar-border">
                    <span class="text-xs text-muted-foreground">
                        <template v-if="ticketTypes.from && ticketTypes.to">
                            Showing {{ ticketTypes.from }}–{{ ticketTypes.to }} of {{ ticketTypes.total }} ticket types
                        </template>
                        <template v-else>
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <span class="px-2 text-xs text-muted-foreground">
                            {{ ticketTypes.current_page }} / {{ ticketTypes.last_page }}
                        </span>
                        <Button
                            variant="outline"
                            size="sm"
                            class="size-8 p-0"
                            :disabled="ticketTypes.current_page >= ticketTypes.last_page"
                            @click="setPage(ticketTypes.current_page + 1)"
                        >
