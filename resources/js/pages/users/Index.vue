...
import {
    index as usersRoute,
    bulk_assign_role as usersBulkAssignRoleRoute,
    bulk_destroy as usersBulkDestroyRoute,
} from '@/routes/users';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <Select
                    ...
                    @update:model-value="
                        (val) =>
                            setFilter('role', val === 'all' ? undefined : val)
                    "
                >
                ...
                <template v-if="selectedCount > 0">
                    <div
                        class="flex items-center gap-2 rounded-md border border-dashed px-3 py-1.5 text-sm"
                    >
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="start">
                                <DropdownMenuLabel
                                    >Assign role to {{ selectedCount }} user{{
                                        selectedCount !== 1 ? 's' : ''
                                    }}</DropdownMenuLabel
                                >
                                <DropdownMenuSeparator />
            ...
            >
                ...
                >
                    <span class="text-xs text-muted-foreground">
                        ...
                        <template v-else> {{ users.total }} users </template>
                    </span>
        ...
        <Dialog v-model:open="showDeleteDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    >
                    <DialogDescription>
                        This action cannot be undone. The selected users will be
                        permanently removed. You cannot delete your own account.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
        ...
        <Dialog v-model:open="showAssignRoleDialog">
            <DialogContent>
                <DialogHeader>
                    <DialogTitle
                        >Assign "{{ pendingRole?.label }}" to
                        {{ selectedCount }} user{{
                            selectedCount !== 1 ? 's' : ''
                        }}?</DialogTitle
                    >
                    <DialogDescription>
                        The "{{ pendingRole?.label }}" role will be added to the
                        selected users. Existing roles are not removed.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button
                    ...
                    <Button @click="executeAssignRole"> Assign role </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    </AppLayout>
