            ...
            <div class="flex items-start justify-between">
                <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center gap-3 text-sm text-muted-foreground">
                        ...
                        <span>&middot;</span>
                        <Badge variant="secondary">{{ eventLabels[webhook.event] ?? webhook.event }}</Badge>
                        <span>&middot;</span>
                        <span>{{ webhook.sent_count.toLocaleString() }} sent</span>
                    </div>
                ...
                <Table class="border">
                    ...
                    <TableBody>
                        <TableRow v-for="delivery in deliveries.data" :key="delivery.id">
                            <TableCell class="text-sm">{{ formatDate(delivery.fired_at) }}</TableCell>
                            <TableCell>
                                <Badge :variant="statusVariant(delivery)">{{ statusLabel(delivery) }}</Badge>
                            </TableCell>
                        ...
                        <TableEmpty v-if="deliveries.data.length === 0" :columns-count="4">
                            No deliveries yet. Deliveries appear here each time this webhook fires.
                        </TableEmpty>
                    </TableBody>
                </Table>

                <!-- Pagination -->
                <div v-if="deliveries.last_page > 1" class="flex items-center justify-between">
                    <span class="text-sm text-muted-foreground">
                        {{ deliveries.from }}–{{ deliveries.to }} of {{ deliveries.total }}
                    </span>
                    <div class="flex items-center gap-2">
                        <Button
                            variant="outline"
                            ...
                            @click="router.visit(webhookShow({ webhook: webhook.id }, { query: { page: deliveries.current_page - 1 } }).url)"
                        >
